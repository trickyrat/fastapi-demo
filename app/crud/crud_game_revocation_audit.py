...
from app.schemas.game_revocation_audit import GameRevocationAuditCreate, GameRevocationAuditUpdate


class CRUDGameRevocationAudit(CRUDBase[GameRevocationAudit, GameRevocationAuditCreate, GameRevocationAuditUpdate]):
    def create(self, db: Session, *, obj_in: GameRevocationAudit) -> GameRevocationAudit:
        db.add(obj_in)
        db.commit()
        db.refresh(obj_in)
        return obj_in
    ...
    def get_all(self, db: Session) -> list[GameRevocationAudit]:
        return db.query(GameRevocationAudit).order_by(GameRevocationAudit.publish_date.desc()).all()

    def multiple_create(self, db: Session, *, objs_in: list[GameRevocationAudit]) -> None:
        db.add_all(objs_in)
        db.commit()
